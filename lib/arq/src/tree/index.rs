...
impl TreeIndex {
    ...
    pub async fn load(&self, commit_id: &SHA1) -> Result<Commit, RepoError> {
        ...
        let commit = self.packset.load(&commit_id).await.and_then(|blob| {
            self.decrypter
        ...
        log::info!(
            "Loading tree from {} (expand key: {}, compression: {:?})",
            commit.tree_sha,
            commit.expand_key,
            commit.compression_type
        );
        let _tree = self.packset.load(&commit.tree_sha).await.and_then(|blob| {
            self.decrypter
                .decrypt_object(&blob.content)
